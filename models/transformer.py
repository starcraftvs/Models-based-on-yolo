...
#一维positional embedding 
class PositionalEncoding(nn.Module):

    def __init__(self, d_hid, n_position=200): #隐藏层维度及最长允许的len
...
class PositionalEnodingSine(nn.Module):
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self,q,k,v,mask=None):
        ...
        attn=self.dropout(torch.softmax(attn,dim=-1)) #softmax再dropout
...
class PositionwiseFeedForward(nn.Module):
    def __init__(self,d_in,d_hid,dropout=0.1):
        ...
        self.layer_norm=nn.LayerNorm(d_in,eps=1e-6) #1e-6代表归一化时分母加1e-6，防止分母为0
        self.dropout=nn.Dropout(dropout)
    #前向
    def forward(self,x):
        residual=x
...
class EncoderLayer(nn.Module):
    ...
    def forward(self,x,slf_attn_mask=None): #输入和self attention的mask
        enc_output,enc_slf_attn=self.slf_attn(q=x,k=x,v=x,mask=slf_attn_mask)
...
class VisionTransformer(nn.Module):
    #最后两个参数
    def __init__(self, n_src_vocab, n_trg_vocab, src_pad_idx, trg_pad_idx,d_word_vec=512, d_model=512, d_inner=2048,n_layers=6, n_head=8, d_k=64, d_v=64, dropout=0.1, n_position=400,trg_emb_prj_weight_sharing=True, emb_src_trg_weight_sharing=True):
        super().__init__()
        #记下下面forward要用的一些参数
        self.src_pad_idx=src_pad_idx
        self.trg_pad_idx=trg_pad_idx
        ...
        #输出映射到词典
        self.resli=nn.Linear(d_model*400,700,bias=False)
    ...
    def forward(self, src_seq, trg_seq):
        ...
        res = self.resli(dec_output) * self.x_logit_scale
        res= res.reshape(res.size(0),100,7)
        #输出结果，一维化成取每个词的概率
...
class Decoder1(nn.Module):
    ...
    def forward(self,trg_seq,trg_mask,enc_output,src_mask,return_attns=False):
        ...
        #过几个decdoer layer
        for dec_layer in self.layer_stack:
            dec_output,dec_slf_attn,dec_enc_attn=dec_layer(dec_output,enc_output,enc_output)
        #记不记录attention
        dec_slf_attn_list += [dec_slf_attn] if return_attns else []
        dec_enc_attn_list += [dec_enc_attn] if return_attns else []
